...
## Rscript --vanilla Rplot.R <sample_of_interest> <tcga_and_gtex_hypoxiaScore File> <sample_hypoxia_scores.tsv> <output_dir> <out_allcancerplot> <out_skinplot>>
...
##  <out_skinplot>                      - output skin plot name
###############################################################################
...
sample_name <- args[1]
tcga_gtexFile <- args[2]
hypoxia_scoreFile <- args[3]
outdir <- args[4]
out_allcancer_boxplot <- args[5]
out_skin_boxplot <- args[6]

sampleList_hypoxiaFiles <- list.files(path=outdir, pattern="tsv$")
...
all_tumors$cancer_type_match=factor(all_tumors$cancer_type_match, levels = c("SKCM", "BLCA", "BRCA", "CHOL", "COAD", "ESCA", "GBM", "HNSC", "KICH", "KIRC", "KIRP", "LIHC", "LUAD", "LUSC", "OV", "PAAD", "PRAD", "READ", "STAD", "THCA", "UCEC"))

cancer_type=c("SKCM", "BLCA", "BRCA", "CHOL", "COAD", "ESCA", "GBM", "HNSC", "KICH", "KIRC", "KIRP", "LIHC", "LUAD", "LUSC", "OV", "PAAD", "PRAD", "READ", "STAD", "THCA", "UCEC")
...
for(hypoFile in sampleList_hypoxiaFiles){
    ...
    df=data.frame(sample_id, hypoxia_score, is_normal)
    datalist[[counter]] <-df
    }
...
sample_IDs = all_samples[which(all_samples$sample_id %in% (big_data[,c(sample_id)])),]
